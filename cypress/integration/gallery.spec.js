describe('Test gallery', () => {
  before(() => {
    ...
    cy.intercept('get', Cypress.env('api_url') + '/admin/gallery', { fixture: 'gallery' })
  })
  it('Visit gallery', () => {
    cy.intercept('post', Cypress.env('api_url') + '/admin/gallery/36', { fixture: 'gallery-destroy' })
    cy.intercept('post', Cypress.env('api_url') + '/admin/gallery', { fixture: 'gallery-store' }).as('gallery.store')
    ...
    cy.wait(300)
    cy.get('.upload-image-box input[type="file"]').selectFile('images/gallery/new-gallery.jpg')
    cy.get('.upload-image-wrap input[type="file"]').selectFile('images/gallery/new-gallery-2.jpg')
    cy.wait(300)
    // ----- 上传图片
